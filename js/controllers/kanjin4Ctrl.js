var kanjin4Ctrls = angular.module('kanjin4Ctrls', []);

kanjin4Ctrls.controller('mainCtrl', function($scope, $http) {
    $scope.course = "kanjin4";
    $scope.kanjistar = 0;
    $scope.kanjiprogress = [];
    ...
    $http({
        ...
        $scope.kanjistar = getCourseStar(data, 'kanjin4');
        $scope.kanjiprogress = getCourseProgress(data, 'kanjin4', 3, 4);
    });
});

kanjin4Ctrls.controller('subCtrl', function($scope, $routeParams, $http) {
    $scope.course = "kanjin4";
    ...
    $http({
        ...
        $scope.kanjistar = getLessonStar(data, 'kanjin4', $routeParams.lessonId);
        $scope.starData = data;
        $scope.enabled = getUnlockedSub($scope.starData, 'kanjin4', $routeParams.lessonId);
    });
 ...
 */
kanjin4Ctrls.controller('learnCtrl', function($scope, $routeParams, $http, dataService) {
    ...
    $scope.removeLife = function() {
        ...
        if ($scope.gameObject.life == 0) {
            gameOver('kanjin4', $routeParams.lessonId, $routeParams.partId, 1, $scope.gameObject.correct);
        }
    };
    ...
    $scope.playSound = function(id, isNormal) {
        ...
        $("audio#" + id + " source").attr("src", "../../data/kanjin4/kanjin4_audio_v2.0/" + $scope.data[id].filename + ".mp3");
 ...
 * @return {[type]}              [description]
 */
kanjin4Ctrls.controller('pictureCtrl', function($scope, $routeParams, $http, $sce, dataService) {
    $scope.course = "kanjin4";
    $scope.lessonId = $routeParams.lessonId;
    $scope.partId = $routeParams.partId;
    ...
    $scope.removeLife = function() {
        ...
        if ($scope.gameObject.life == 0) {
            gameOver("kanjin4", $routeParams.lessonId, $routeParams.partId, 2, $scope.gameObject.correct);
        }
    };
 ...
 * @return {[type]}              [description]
 */
kanjin4Ctrls.controller('wordCtrl', function($scope, $routeParams, $http, dataService) {
    $scope.lessonId = $routeParams.lessonId;
    $scope.partId = $routeParams.partId;
    $scope.choices = [];
    $scope.step = 0;
    ...
    $scope.removeLife = function() {
        $scope.gameObject.life--;
        if ($scope.gameObject.life == 0) {
            gameOver("kanjin4", $routeParams.lessonId, $routeParams.partId, 3, $scope.gameObject.correct);
        }
    };
 ...
 * @return {[type]}              [description]
 */
kanjin4Ctrls.controller('connectCtrl', function($scope, $routeParams, $http, dataService) {
    $scope.lessonId = $routeParams.lessonId;
    $scope.partId = $routeParams.partId;
    $scope.gameObject = {
        "life": 3,
    ...
    $scope.removeLife = function() {
        ...
        if (angular.equals($scope.gameObject.life, 0)) {
            gameOver("kanjin4", $routeParams.lessonId, $routeParams.partId, 5, $scope.gameObject.correct * 2);
        }
    };
